...
int main(int argc, char **argv)
{
			...
			for (const auto &archive : archives)
			{
				...
				for (const auto &resourceID : arch.GetResourceIDs())
				{
					...
					for (const auto &streamIndex : arch.GetResourceStreamIndices(resourceID))
					{
						...
						if (!resource)
						{
							std::cout << "Skipping " << idStr << ": " << arch.GetLastErrorMessage() << '\n';
							continue;
						}
	...
	else
	{
		...
		{
			std::cout << "Refusing to overwrite existing output file " << outputPath.string() << '\n';
			return EXIT_FAILURE;
		}
		...
		if (std::filesystem::is_regular_file(projectMetadataPath) || std::filesystem::is_symlink(projectMetadataPath))
		{
			...
			std::cout << "Packed " << packDir.string() << " to " << outputPath.string() << '\n';
			return EXIT_SUCCESS;
#else
			std::cout << "This build of bndl_util was compiled without project import support.\n";
			return EXIT_FAILURE;
#endif
		}
		...
		{
			std::cout << "Legacy _config.xml packing is not implemented. Export a libtub project with .meta.yaml and pack that directory instead.\n";
			return EXIT_FAILURE;
		}

		std::cout << "Could not find a .meta.yaml project file in the directory to pack.\n";
		return EXIT_FAILURE;
	}
